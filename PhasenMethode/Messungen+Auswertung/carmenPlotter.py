...
CSTM_TITLE = "\t29k/57k-Frequencies with max DDS-Amplitudes (Board_v1+VccLabor)" #Anti-Aliasing T=2min, step=1bar/10s, 0-10bar 
NUM_FREQUENCIES = 9
NUM_MOV_AVG = 100
frequencies = [ 57600,
                52662.86,
...
with open(fileName,'r', newline='') as csvfile:
    ...
    for row in ax:
        for col in row:
            ...
            # Moving average
            col.plot(xticks[int(NUM_MOV_AVG/2) - 1 : -int(NUM_MOV_AVG/2)], moving_average(Cr_A_m[i], NUM_MOV_AVG))
            col.plot(xticks[int(NUM_MOV_AVG/2) - 1 : -int(NUM_MOV_AVG/2)], moving_average(Cp_A_m[i], NUM_MOV_AVG))

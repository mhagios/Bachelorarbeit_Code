...
fileDir = 'Messungen/03_11_NeuesBoard_Messplatz'
    ...
    for file in files:
        ...
        with open(str(root) + "/"+ file,'r', newline='') as csvfile:
            ...
            for i in range(NUM_FREQUENCIES): 
                ...
                Cr_A_sigma[i] = [np.mean(Cr_A_m[i][x* pressureLen + windowLow:
                ...
                for x in range(NUM_FREQUENCIES):
                    if math.isnan(Cr_A_sigma[i][x]):
                        Cr_A_sigma[i][x] = 0
                
                #Wiederholen für Cr/Cp bei Amplitude/Phase
                Cr_P_sigma[i] = [np.mean(Cr_P_m[i][x* pressureLen + windowLow:
                                                    x* pressureLen + windowHigh])
                        for x in range(NUM_PRESSURE_STEPS - 1)]
                for x in range(NUM_FREQUENCIES):
                    if math.isnan(Cr_P_sigma[i][x]):
                        Cr_P_sigma[i][x] = 0
                
                Cp_A_sigma[i] = [np.mean(Cp_A_m[i][x* pressureLen + windowLow:
                                                    x* pressureLen + windowHigh])
                        for x in range(NUM_PRESSURE_STEPS - 1)]
                for x in range(NUM_FREQUENCIES):
                    if math.isnan(Cp_A_sigma[i][x]):
                        Cp_A_sigma[i][x] = 0
                
                Cp_P_sigma[i] = [np.mean(Cp_P_m[i][x* pressureLen + windowLow:
                                                    x* pressureLen + windowHigh])
                        for x in range(NUM_PRESSURE_STEPS - 1)]
                for x in range(NUM_FREQUENCIES):
                    if math.isnan(Cp_P_sigma[i][x]):
                        Cp_P_sigma[i][x] = 0
...
for row in axs:
    for col in row:
        if f >= 9:
            break
        #Cr_A_sigma_m:  1. Dataset
        #               2. Frequency
        #               3. PressureStep
        Cr_A_plot = Cr_A_sigma_m[0]
        Cp_A_plot = Cp_A_sigma_m[0]
        Cv = [(Cp_A_plot[f][p] - Cr_A_plot[f][p]) / Cp_A_plot[f][p] for p in range(NUM_PRESSURE_STEPS - 1)]
        col.plot(Cv)
        col.plot(np.linspace((Cv[0]), 
                              Cv[NUM_PRESSURE_STEPS - 2], 
